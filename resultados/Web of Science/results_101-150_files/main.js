...
$(".main-container").click(function (event) {
	...
	if($target.is(".icon-dd-active-breadcrumb") || $target.is("#databases")){
		if ($linkStatus) {
...
$(".main-container").click(function (event) {
	...
	if($target.is(".icon-dd-active-block-search") || $target.is("#searchTypes") || $target.is("#historyDatabases")){
		if ($searchStatus) {
...
$('.ico-close-alert').click(function(){
	...
	// $(this).parent().parent.id; // Save this id as "acknowledged" so we don't display it again.
});		
...
/* Moved this to the search_input.jsp so we don't do it on every page. */
/*
...
/* if the browser is not support placeholder for input,then we trigger our handling*/
if (!$.fn.placeholder.input) {
	$('input').placeholder();
}
/* if the browser is not support placeholder for textarea,then we trigger our handling*/
if (!$.fn.placeholder.textarea) {
